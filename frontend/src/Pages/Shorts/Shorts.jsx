...
function Shorts() {
  ...
  useEffect(() => {
    ...
    const suffled = [...allShortsData].sort(() => Math.random() - 0.5);
  ...
  useEffect(() => {
    const addhistory = async () => {
      try {
        const shortId = shortList[activeIndex];
        if (!shortId) {
          return;
        }
        const result = await axios.post(
          `${serverUrl}/api/user/addhistory`,
          { contentid: shortId._id, contenttype: "Short" },
    ...
    <div className="h-[calc(100vh-95px)] w-full overflow-y-scroll snap-y snap-mandatory md:mt-13">
      {shortList?.map((short, index) => {
        return (
          <div
            key={short._id}
            className="h-full w-full flex md:items-center items-start justify-center snap-start pt-10 md:pt-0"
          >
            <div className="relative w-[420px] md:w-[350px]  md:aspect-9/16 sm:aspect-10/15 aspect-10/15 bg-black rounded-2xl overflow-hidden shadow-xl border-gray-700 cursor-pointer">
              <video
                src={short?.shorturl}
